...
if ($selected.ContainsKey('Minor')) {
    $report.SdkFull = Invoke-LabAdb @('-s',$selected.Serial,'shell','getprop','ro.build.version.sdk_full')
}
...
if ($selected.ContainsKey('Minor') -and $report.SdkFull -ne "$($selected.Api).$($selected.Minor)") { throw "Expected Android SDK $($selected.Api).$($selected.Minor). Observed $($report.SdkFull)." }
...
if ($Profile -in @('Api36', 'Api361', 'Api37') -and $report.AdbUid -eq '0') { throw "Clean $Profile baseline unexpectedly has root ADB. Investigate without repatching." }
