...
For your own test APK, copy `config/api36.yaml` to a local YAML file and
set its real `app.package`, `app.activity`, and optional `ready_activity`:

```powershell
uv run lab --config config/api36.yaml install --apk C:/path/to/your-app.apk
uv run lab --config config/local-app.yaml smoke
```
...
Continue with [the deployment guide](docs/COMMANDS.md) when a host is available.
