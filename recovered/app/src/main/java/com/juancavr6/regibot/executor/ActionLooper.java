...
public class ActionLooper implements Runnable {
    ...
    public volatile Bitmap lastScreenShot;
    ...
    public void run() {
        try {
            ...
            if (!isRunning || Thread.currentThread().isInterrupted()) return;
            ...
            if (FloatingMenuService.instance != null) {
                service.mainHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        if (isRunning && FloatingMenuService.instance != null) {
            ...
            while (isRunning){
                synchronized (pauseLock) {
                    while (isRunning && isPaused) pauseLock.wait();
                }
                if (!isRunning) break;
                if(!isPaused){
                    try {
